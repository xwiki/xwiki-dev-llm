...
| `JIRA_API_TOKEN`        | `xwiki-jira` (jira-cli / REST), `xwiki-jira-bfd` | Your jira.xwiki.org personal access token. Optional — only needed to act on JIRA issues. See "JIRA access" below. |
| `TYPESAFE_TOKEN`        | `xwiki-jira-bfd` | A [typesafe.ai](https://docs.typesafe.ai) API key for jev, the calibrated judge the BFD triage was measured on (`TYPESAFE_API_KEY` also works). Optional — without it the skill offers Claude as the judge through the `claude` CLI, at your Claude usage's cost, and proposes fewer kinds of close. |
| `BFD_JUDGE`, `CLAUDE_JUDGE_MODEL` | `xwiki-jira-bfd` | Force the judge (`jev` or `claude`), and pick the Claude model (default `sonnet`). Optional. A Claude model other than the one measured (see the skill's `tools/eval/RESULTS.md`) proposes no closes at all. |
